...
// Khariidadda sawirrada ee alaabta Underwear Group-ka ah
const imageMap = {
  "buumo": "/images/short.png",
  "underwear": "/images/short.png",
  "سروال داخلي قصير": "/images/short.png",
  "funaanad hoose": "/images/funanadhose.png",
  "undershirt": "/images/funanadhose.png",
  "فنيلة داخلية": "/images/funanadhose.png",
  "garan": "/images/garan.png",
  "nigis": "/images/nigis.png",
  "سروال داخلي": "/images/nigis.png",
  "surwaal hoose": "/images/surwalhore.png",
  "سروال داخلي طويل": "/images/surwalhore.png",
  "default": "/images/underwear-item.png" 
};

export default function UnderwearServicesPage() {
  ...
  const getProductImage = (name) => {
    ...
    if (pName.includes("buumo") || pName.includes("سروال داخلي قصير")) return imageMap["buumo"];
    if (pName.includes("garan")) return imageMap["garan"];
    if (pName.includes("funaanad hoose") || pName.includes("undershirt") || pName.includes("فنيلة داخلية")) return imageMap["funaanad hoose"];
    if (pName.includes("surwaal hoose") || pName.includes("سروال داخلي طويل")) return imageMap["surwaal hoose"];
    if (pName.includes("nigis") || pName.includes("سروال داخلي") || pName.includes("underwear")) return imageMap["nigis"];
  ...
  const processedProducts = useMemo(() => {
    // Liiska alaabta target-ka ah ee Underwear Group
    const targetItems = [
      "buumo", "underwear", "سروال داخلي قصير",
      "funaanad hoose", "undershirt", "فنيلة داخلية",
      "garan",
      "nigis", "سروال داخلي",
      "surwaal hoose", "سروال داخلي طويل"
    ];
    ...
    rawProducts.forEach(prod => {
      ...
      const isUnderwearItem = targetItems.some(target => pName.includes(target));
      if (!isUnderwearItem) return;
      ...
      let finalKey = "other";
      if (pName.includes("buumo") || pName.includes("سروال داخلي قصير")) {
        finalKey = "buumo";
      } else if (pName.includes("garan")) {
        finalKey = "garan"; 
      } else if (pName.includes("funaanad hoose") || pName.includes("undershirt") || pName.includes("فنيلة داخلية")) {
        finalKey = "funaanad hoose";
      } else if (pName.includes("surwaal hoose") || pName.includes("سروال داخلي طويل")) {
        finalKey = "surwaal hoose";
      } else if (pName.includes("nigis") || pName.includes("underwear") || pName.includes("سروال داخلي")) {
        finalKey = "nigis";
      }
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="text-center">
        ...
        <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-slate-400">Loading Underwear Collection...</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="text-center px-6">
        ...
        <button 
          ...
          className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold uppercase text-xs tracking-widest hover:bg-indigo-700 shadow-lg shadow-indigo-500/20 transition-all duration-300"
        >
          Try Again
        </button>
      </div>
    ...
    <main className="min-h-screen bg-[#FAFAFC]">
      ...
      <section className="relative h-[420px] flex items-center justify-center bg-slate-950 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <div className="absolute inset-0 bg-gradient-to-b from-indigo-950/20 via-slate-950/80 to-slate-950 z-10" />
          <motion.img 
            initial={{ scale: 1.15 }}
            animate={{ scale: 1 }}
            transition={{ duration: 12, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" }}
            src="/images/underwear-item.png" 
        ...
        <div className="relative z-20 text-center px-6">
          ...
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.25em] text-indigo-200">LikeNew Hygiene</span>
          </motion.div>
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tight text-white mb-3">
            UNDERWEAR <span className="text-indigo-500 italic font-serif">CARE</span>
          </h1>
          <p className="text-slate-400 text-xs md:text-sm max-w-md mx-auto tracking-wide font-medium opacity-90">
            Hygienic, fresh, and deeply sanitized cleaning process specialized for everyday essentials.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 -mt-10 relative z-30">
        <div className="bg-white p-2 rounded-3xl shadow-2xl border border-slate-100 flex items-center px-6 group focus-within:ring-4 ring-indigo-500/10 transition-all">
          <Search className="text-slate-400 mr-3 group-focus-within:text-indigo-600" size={20} />
          <input 
            type="text"
            placeholder="Search underwear services..."
                  ...
                  <div>
                    ...
                    <div className="px-4 pb-2">
                      <div className="flex justify-between items-start mb-1 gap-2">
                        ...
                        <span className="font-black text-sm text-indigo-600 whitespace-nowrap">
                          ${Number(item.price || 0).toFixed(2)}
                        </span>
                      </div>

                      <p className="text-[9px] font-bold text-indigo-500 uppercase tracking-widest mb-6">
