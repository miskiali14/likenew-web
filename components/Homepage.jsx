...
export default function HomePage() {
  ...
  const getHomeGroupKey = (name) => {
    ...
    if (
      pName.includes("caga-saar weyn") ||
      pName.includes("caga saar weyn") ||
      pName.includes("cagasaar weyn")
    )
      return "caga_saar_weyn";

    if (
      pName.includes("foodaraha kuraasta") ||
      pName.includes("foodare kursi")
    )
      return "foodaraha_kuraasta";

    if (
      pName.includes("caga-saar yar") ||
      pName.includes("caga saar yar") ||
      pName.includes("cagasaar yar")
    )
      return "caga_saar_yar";

    if (
      pName.includes("daah") ||
      pName.includes("curtain") ||
      pName.includes("ستارة")
    )
      return "daah_curtain";
    ...
    if (
      pName.includes("maro fadhi (full)") ||
      pName.includes("maro fadhi full")
    )
      return "maro_fadhi_full";

    if (
      pName.includes("maro fadhi haff") ||
      pName.includes("maro fadhi haf") ||
      pName.includes("haff")
    )
      return "maro_fadhi_haff";
    ...
    rawProducts.forEach((prod) => {
      ...
      if (!uniqueMap.has(groupKey)) {
        uniqueMap.set(groupKey, {
          id: prod.productID || prod.id || groupKey,
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="relative h-[420px] flex items-center justify-center bg-slate-950 overflow-hidden">
        ...
        <div className="relative z-20 text-center px-6">
          ...
          >
            <Star size={14} className="text-[#7047A8] fill-[#7047A8]" />
          ...
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tight text-white mb-3">
            HOME <span className="text-[#7047A8] italic font-serif">CARE</span>
          </h1>

          <p className="text-slate-400 text-xs md:text-sm max-w-md mx-auto tracking-wide font-medium opacity-90">
            Premium cleaning for carpets, curtains, sofa covers, prayer mats and home essentials.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 pb-40">
        <div className="mb-16 max-w-md mx-auto">
          <div className="relative group">
            <Search
              className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#7047A8]"
              size={18}
            />
              ...
              {filteredProducts.map((item) => (
                ...
                >
                  <div className="text-center">
                    ...
                    <p className="text-[11px] text-gray-500 leading-tight">
                      Ready in:{" "}
                      <span className="text-black font-bold">from 1 day</span>
                    </p>
                  </div>
