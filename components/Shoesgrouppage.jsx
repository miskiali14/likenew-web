...
// Khariidadda sawirrada ee alaabta Shoes Group-ka ah (6 Nooc oo kaliya)
const imageMap = {
...
export default function ShoesServicesPage() {
  ...
  const getProductImage = (name) => {
    ...
    if (pName.includes("kabo ciidan")) return imageMap["kabo ciidan"];
    if (pName.includes("kabo dumar")) return imageMap["kabo dumar"];
    if (pName.includes("kabo sneaker")) return imageMap["kabo sneaker"];
    if (pName.includes("kabo sports")) return imageMap["kabo sports"];
    if (pName.includes("kabo suud")) return imageMap["kabo suud"];
  ...
  const processedProducts = useMemo(() => {
    // 6-DA NOOC EE KABAHA AH OO KALIYA (WAA LAGA SAARAY FUNAANAD, FUTASHAARI, IYO BOORSO)
    const targetItems = [
      "kabo ciidan",
      "kabo dumar",
      "kabo sneaker",
    ...
    rawProducts.forEach(prod => {
      ...
      // Hubi haddii magacu uu si sax ah u jiro mid ka mid ah 6-da kabood
      const matchedTarget = targetItems.find(target => pName.includes(target));
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="text-center">
        ...
        <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-slate-400">Loading Shoes Collection...</p>
      </div>
    </div>
  );
    ...
    <main className="min-h-screen bg-[#FAFAFC]">
      ...
      <section className="relative h-[420px] flex items-center justify-center bg-slate-950 overflow-hidden">
        ...
        <div className="relative z-20 text-center px-6">
          ...
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tight text-white mb-3">
            SHOES <span className="text-[#6a3da1] italic font-serif">CARE</span>
          </h1>
          <p className="text-slate-400 text-xs md:text-sm max-w-md mx-auto tracking-wide font-medium opacity-90">
            Professional footwear cleaning, stain removal, leather conditioning, and deep deodorization.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 -mt-10 relative z-30">
        <div className="bg-white p-2 rounded-3xl shadow-2xl border border-slate-100 flex items-center px-6 group focus-within:ring-4 ring-purple-500/10 transition-all">
          ...
          <input 
            type="text"
            placeholder="Search shoes services..."
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 pb-40">
        {filteredProducts.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-[2rem] border border-slate-100 shadow-sm max-w-md mx-auto">
            <p className="text-slate-400 font-medium">No shoes services found for this tab.</p>
          </div>
        ) : (
