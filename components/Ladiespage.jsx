...
function LadiesServicesContent() {
  ...
  const getProductImage = (name) => {
    ...
    if (
      pName.includes("aroos") ||
      pName.includes("saako aroos") ||
      pName.includes("saako-aroos")
    ) {
  ...
  const processedProducts = useMemo(() => {
    ...
    rawProducts.forEach((prod) => {
      ...
      if (activeSection === "30") {
        ...
        if (
          !strictPressOnlyItems.some(
            (allowedItem) =>
              pName === allowedItem ||
              pName.includes(allowedItem)
          )
        )
          return;
      ...
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center px-6">
          ...
          <button
            ...
            className="bg-[#7047A8] text-white px-8 py-3 rounded-xl font-bold uppercase text-xs tracking-widest hover:bg-[#5d3990] transition-all duration-300"
          >
            Try Again
          </button>
        </div>
      ...
      <section className="relative h-[420px] flex items-center justify-center bg-slate-950 overflow-hidden">
        ...
        <div className="relative z-20 text-center px-6">
          ...
          <p className="text-slate-400 text-xs md:text-sm max-w-md mx-auto tracking-wide font-medium opacity-90">
            Premium eco-friendly care and premium cleaning
            tailored for your finest garments.
          </p>
        </div>
      </section>
            ...
            <AnimatePresence mode="popLayout">
              {filteredProducts.map((item) => (
                <motion.div
                  ...
                  className="group bg-[#F3F3F3] rounded-[14px] min-h-[318px] p-6 flex flex-col justify-between transition-all duration-300"
                >
                  ...
                  <div className="flex-1 flex items-center justify-center py-4">
                    <img
                      ...
                      className="max-h-[135px] object-contain group-hover:scale-105 transition-transform duration-500 ease-out"
                      onError={(e) => {
