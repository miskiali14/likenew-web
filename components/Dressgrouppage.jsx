...
function DressServicesContent() {
  ...
  const getProductImage = (name) => {
    ...
    if (
      pName.includes("african dress") ||
      pName.includes("زي افريقي")
    )
      return imageMap["african dress"];

    if (
      pName.includes("aviation dress") ||
      pName.includes("زي طيران مدني")
    )
      return imageMap["aviation dress"];

    if (
      pName.includes("xirfadlayasha") ||
      pName.includes("المحترفين")
    )
      return imageMap["isku-joga xirfadlayasha"];

    if (
      pName.includes("waxbarasho") ||
      pName.includes("uniform") ||
      pName.includes("مدرسي")
    )
      return imageMap["isku-joog waxbarasho"];

    if (pName.includes("ixraam") || pName.includes("احرام"))
      return imageMap.ixraam;

    if (
      pName.includes("dhaqaatiirta") ||
      pName.includes("doctor") ||
      pName.includes("طبيب")
    )
      return imageMap["jaakada dhaqaatiirta"];

    if (
      pName.includes("military") ||
      pName.includes("عسكري")
    )
      return imageMap["military dress"];

    if (pName.includes("police") || pName.includes("شرطي"))
      return imageMap["police dress"];

    if (pName.includes("traffic") || pName.includes("مرور"))
      return imageMap["traffic dress"];
  ...
  const processedProducts = useMemo(() => {
    ...
    rawProducts.forEach((prod) => {
      ...
      const isDressItem = targetItems.some((target) =>
      ...
      <section className="relative h-[420px] flex items-center justify-center bg-slate-950 overflow-hidden">
        ...
        <div className="relative z-20 text-center px-6">
          ...
          <p className="text-slate-400 text-xs md:text-sm max-w-md mx-auto tracking-wide font-medium opacity-90">
            Premium cleaning and professional care for uniforms,
            workwear and special garments.
          </p>
        </div>
      </section>
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-16 bg-[#F3F3F3] rounded-[14px] max-w-md mx-auto">
            <p className="text-slate-400 font-medium">
              No dress or uniform services found for this tab.
            </p>
          </div>
        ) : (
                ...
                >
                  <div className="text-center">
                    ...
                    <p className="text-[11px] text-gray-500 leading-tight">
                      Professional dress and uniform cleaning
                    </p>
