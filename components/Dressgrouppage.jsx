...
// Khariidadda sawirrada ee alaabta Dress Group-ka ah
const imageMap = {
  "african dress": "/images/dharkafrica.png",
  "aviation dress": "/images/dharkaviation.png",
  "isku-joga xirfadlayasha": "/images/xirfadlayaasha.png",
  "isku-joog waxbarasho": "/images/uniform.png",
  "ixraam": "/images/ixraam.png",
  "jaakada dhaqaatiirta": "/images/DoctorJacket.png",
  "military dress": "/images/MilitaryDress.png",
  "police dress": "/images/PoliceDress.png",
  "traffic dress": "/images/TrafficDress.png",
  "default": "/images/TrafficDress.png" 
};

export default function DressServicesPage() {
  ...
  const getProductImage = (name) => {
    ...
    if (pName.includes("african dress") || pName.includes("زي افريقي")) return imageMap["african dress"];
    if (pName.includes("aviation dress") || pName.includes("زي طيران مدني")) return imageMap["aviation dress"];
    if (pName.includes("xirfadlayasha") || pName.includes("المحترفين")) return imageMap["isku-joga xirfadlayasha"];
    if (pName.includes("waxbarasho") || pName.includes("uniform") || pName.includes("مدرسي")) return imageMap["isku-joog waxbarasho"];
    if (pName.includes("ixraam") || pName.includes("احرام")) return imageMap["ixraam"];
    if (pName.includes("dhaqaatiirta") || pName.includes("doctor") || pName.includes("طبيب")) return imageMap["jaakada dhaqaatiirta"];
    if (pName.includes("military") || pName.includes("عسكري")) return imageMap["military dress"];
    if (pName.includes("police") || pName.includes("شرطي")) return imageMap["police dress"];
    if (pName.includes("traffic") || pName.includes("مرور")) return imageMap["traffic dress"];
  ...
  const processedProducts = useMemo(() => {
    // Liiska rasmiga ah ee alaabta Dress & Uniform Group ee la rabo kaliya
    const targetItems = [
      "african dress", "زي افريقي",
      "aviation dress", "زي طيران مدني",
      "isku-joga xirfadlayasha", "زي المحترفين",
      "isku-joog waxbarasho", "uniform", "زي مدرسي",
      "ixraam", "احرام",
      "jaakada dhaqaatiirta", "doctor jacket", "جاكيت طبيب",
      "military dress", "زي عسكري",
      "police dress", "زي شرطي",
      "traffic dress", "زي مرور"
    ];
    ...
    rawProducts.forEach(prod => {
      ...
      // Hubi haddii alaabtu ay ku jirto liiska kor ku xusan ee xaddidan
      const isDressItem = targetItems.some(target => pName.includes(target));
      if (!isDressItem) return;
      ...
      let finalKey = "other";
      if (pName.includes("african dress") || pName.includes("زي افريقي")) {
        finalKey = "african dress";
      } else if (pName.includes("aviation dress") || pName.includes("زي طيران مدني")) {
        finalKey = "aviation dress";
      } else if (pName.includes("xirfadlayasha") || pName.includes("المحترفين")) {
        finalKey = "isku-joga xirfadlayasha";
      } else if (pName.includes("waxbarasho") || pName.includes("uniform") || pName.includes("مدرسي")) {
        finalKey = "isku-joog waxbarasho";
      } else if (pName.includes("ixraam") || pName.includes("احرام")) {
        finalKey = "ixraam";
      } else if (pName.includes("dhaqaatiirta") || pName.includes("doctor") || pName.includes("طبيب")) {
        finalKey = "jaakada dhaqaatiirta";
      } else if (pName.includes("military") || pName.includes("عسكري")) {
        finalKey = "military dress";
      } else if (pName.includes("police") || pName.includes("شرطي")) {
        finalKey = "police dress";
      } else if (pName.includes("traffic") || pName.includes("مرور")) {
        finalKey = "traffic dress";
      }
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="text-center">
        ...
        <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-slate-400">Loading Dress & Uniform Collection...</p>
      </div>
    </div>
  );
    ...
    <main className="min-h-screen bg-[#FAFAFC]">
      ...
      <section className="relative h-[420px] flex items-center justify-center bg-slate-950 overflow-hidden">
        ...
        <div className="relative z-20 text-center px-6">
          ...
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tight text-white mb-3">
            DRESS & UNIFORM <span className="text-[#6a3da1] italic font-serif">CARE</span>
          </h1>
          <p className="text-slate-400 text-xs md:text-sm max-w-md mx-auto tracking-wide font-medium opacity-90">
            Premium cleaning and institutional care for uniforms, traditional garments, and specialized workwear.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 -mt-10 relative z-30">
        <div className="bg-white p-2 rounded-3xl shadow-2xl border border-slate-100 flex items-center px-6 group focus-within:ring-4 ring-purple-500/10 transition-all">
          ...
          <input 
            type="text"
            placeholder="Search dress & uniform services..."
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 pb-40">
        {filteredProducts.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-[2rem] border border-slate-100 shadow-sm max-w-md mx-auto">
            <p className="text-slate-400 font-medium">No dress or uniform services found for this tab.</p>
          </div>
        ) : (
